...
import com.jzk.qrcodelibrary.decode.Utils;
import com.jzk.qrcodelibrary.statusutils.StatusBarUtil;
...
public final class CommonScanActivity extends Activity implements ScanListener, View.OnClickListener {
    ...
    public void onCreate(Bundle icicle) {
        ...
        StatusBarUtil.setColor(this, Color.parseColor("#ffb700"));
        ...
        pointMsg = intent.getStringExtra("pointMsg");
        initView();
    }
...
//		Toast.makeText(that, "result="+rawResult.getText(), Toast.LENGTH_LONG).show();
        ...
        if (resultString.equals("")) {
            Toast.makeText(this, pointMsg, Toast.LENGTH_SHORT).show();
        } else {
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.qrcode_g_gallery) {
            showPictures(PHOTOREQUESTCODE);
        } else if (v.getId() == R.id.iv_light) {
            scanManager.switchLight();
        } else if (v.getId() == R.id.qrcode_ic_back) {
            finish();
        } else if (v.getId() == R.id.service_register_rescan) {
            startScan();
